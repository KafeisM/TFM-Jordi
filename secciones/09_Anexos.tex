...
\begin{listing}[style=consola,caption={Ejemplo de entrenamiento supervisado con reciclado preventivo, dos drones y \emph{warm start} de la etapa anterior del curriculum.},label={lst:anexo:supervise_train}]
RECYCLE_SECONDS=9000 \
DRONE_NAMESPACES=drone0,drone1 \
INITIAL_RESUME=runs/ppo/<run_exp016>/model_final.zip \
bash scripts/supervise_train.sh configs/train_ppo_phase1_exp018.yaml
\end{listing}

La evaluación formal y la comparación entre controladores se ejecutan con el
mismo punto de entrada, variando únicamente el controlador y el modo:

\begin{listing}[style=consola,caption={Evaluación por semillas de la política y del PID de referencia sobre episodios idénticos.},label={lst:anexo:evaluate}]
...
python3 scripts/evaluate_policy.py \
  --config configs/train_ppo_phase1_exp016.yaml \
  --controller pid --episodes 100 --seed 1000
\end{listing}
...
desarrollo en la fase final del trabajo, con apoyo a la campaña experimental
(supervisión de entrenamientos, análisis de registros y diagnóstico de fallos de
integración), a la implementación asistida de componentes con sus pruebas y a la
redacción de secciones de la memoria a partir de los datos experimentales
...
Claude Code & Apoyo a la campaña experimental, implementación asistida con pruebas automatizadas, generación de figuras desde los datos y redacción de secciones de la memoria. & Pruebas automatizadas en verde, revisión adversarial previa a cada integración, verificación de cifras contra los registros experimentales y revisión manual del texto. \\
\hline
\end{tabular}
