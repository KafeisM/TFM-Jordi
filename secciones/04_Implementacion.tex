...
\hline
\texttt{rl\_uav/envs/as2\_test\_env.py} & Implementación del entorno Gymnasium y puente con Aerostack2. \\
\texttt{rl\_uav/training/ppo\_setup.py} & Construcción de entorno vectorizado, rutas de ejecución y modelo PPO. \\
\texttt{configs/train\_ppo.yaml} & Configuración declarativa de entorno, entrenamiento e hiperparámetros. \\
\texttt{scripts/train\_ppo.py} & Punto de entrada para entrenamiento y comprobación de configuración. \\
\texttt{scripts/as2\_runtime.py} & Comprobación y relanzamiento controlado del simulador AS2. \\
\texttt{scripts/supervise\_train.sh} & Supervisor de entrenamientos largos con reinicio del stack AS2 y reanudación desde \emph{checkpoint}. \\
\texttt{as2\_platform\_multirotor\_simulator} & Fork del simulador con servicio de reinicio determinista del estado del multirrotor. \\
\texttt{as2\_sim/launch\_sim.bash} & Arranque del simulador monodron o multidron mediante nombres de dron. \\
\texttt{scripts/test\_*} y \texttt{scripts/validate\_*} & Validaciones locales y reales del entorno, la vectorización y la integración. \\
\hline
\end{tabular}
...
Desde el punto de vista de la memoria, lo relevante no es enumerar todos los
...
modo \emph{offboard} y en estado de vuelo. Esta funcionalidad es la base que
permite que Exp009 actúe como gate de certificación del reset antes de lanzar
experimentos PPO más largos.
...
A partir de Exp010 se emplea una configuración más cercana a la especificación
experimental indicada por el tutor, con tasa de aprendizaje \texttt{3.0e-05},
...
evalúan las condiciones de terminación y se generan campos de diagnóstico. Las
configuraciones Exp010--Exp012 usan una recompensa mínima basada en distancia,
bonificación terminal de éxito y penalización por condiciones de seguridad; los
términos auxiliares de progreso, \emph{path-facing} y seguridad vertical quedan
desactivados para no mezclar el análisis de la tarea con shaping adicional.
...
La implementación añade instrumentación específica porque el entrenamiento solo
...
internos de reinicio de la velocidad máxima permitida a PPO. También se incorpora
\texttt{reset\_yaw\_required}, que permite tratar la guiñada como requisito
bloqueante o como diagnóstico según la fase experimental. Cuando la interfaz
acepta comandos pero el UAV no se desplaza de forma apreciable, el entorno falla
...
el mecanismo habitual para corregir a un agente aún no entrenado.
...
\texttt{drone1}, \texttt{drone2} y \texttt{drone3}, todas conectadas al mismo
...
largas, \texttt{train\_ppo.py} acepta \texttt{--resume-from} y
\texttt{scripts/supervise\_train.sh} automatiza el reinicio del stack AS2, la
comprobación del servicio de simulación y la reanudación desde el último
\emph{checkpoint}. Esta capa no modifica PPO, pero hace más robusta la ejecución
experimental.
