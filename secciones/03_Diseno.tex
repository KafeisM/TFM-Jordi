...
embargo, esos términos se desactivan en Exp010--Exp012 para evitar que el agente
optimice una recompensa de shaping distinta de la tarea principal. La guiñada se mantiene como requisito terminal suave. Cuando el UAV alcanza la
...
La expresión final aplicada en las configuraciones Exp010--Exp012 incorpora las
...
Desde el punto de vista del diseño de ejecución, \texttt{SyncVectorEnv} ofrece
un mecanismo directo de ejecución síncrona en el que las instancias avanzan de
forma coordinada. En el contexto de Stable-Baselines3, el diseño contempla
\texttt{DummyVecEnv} y \texttt{SubprocVecEnv} como alternativas configurables. La
opción síncrona, equivalente en este caso a \texttt{DummyVecEnv}, constituye el
camino prudente cuando se comparte un simulador y se desea minimizar la
complejidad de concurrencia. \texttt{SubprocVecEnv} puede
aportar aislamiento por proceso, pero también introduce mayores exigencias sobre
la inicialización de ROS 2 y la gestión de recursos compartidos.

La decisión principal de diseño consiste, por tanto, en priorizar la corrección de
la interacción sobre la paralelización agresiva. En un sistema donde varias
instancias comparten simulador, una vectorización incorrecta puede producir
interferencias entre drones, bloqueos por inicialización repetida o estados de
episodio difíciles de atribuir. Por ello, la arquitectura favorece una
vectorización controlada, con espacios de nombres explícitos y una inicialización
de ROS 2 protegida por proceso, antes que asumir incrementos de rendimiento no
verificados.
